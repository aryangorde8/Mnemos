...
Pipeline (same order as the TS tool):
  ...
  4. Optional LLM rerank over the top candidates
"""
...
from app.llm.llm_client import embed_query, generate
...
async def search_memory(query: str, limit: int = 8, source: str | None = None,
    ...
    try:
        ...
        phases = [
            ...
            f"rrf → {len(merged)}",
            f"rerank · llm · top {len(final)}" if reranked else f"top {len(final)}",
        ]
